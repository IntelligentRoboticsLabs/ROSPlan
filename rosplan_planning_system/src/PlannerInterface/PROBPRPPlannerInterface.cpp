...
#include <sys/time.h>*/
#include "rosplan_planning_system/PlannerInterface/PROBPRPPlannerInterface.h"
#include <utility>
#include <string>
#include <vector>
...
namespace KCL_rosplan
{
  ...
  /* constructor */
  /*-------------*/
  PROBPRPPlannerInterface::PROBPRPPlannerInterface(ros::NodeHandle& nh)
  {
    node_handle = &nh;
    plan_server = new actionlib::SimpleActionServer<rosplan_dispatch_msgs::PlanAction>
      (
        (*node_handle),
        "start_planning",
  ...
  PROBPRPPlannerInterface::~PROBPRPPlannerInterface()
  {
    delete plan_server;
  }

  void PROBPRPPlannerInterface::loadParams()
  {
    node_handle->getParam("problem_file", problem_file);
    node_handle->getParam("domain_file", domain_file);
    node_handle->getParam("domainfo_file", domainfo_file);
    node_handle->getParam("planner_path", planner_path);
    node_handle->getParam("script_path", script_path);
    node_handle->getParam("translator_file", translator_file);
    node_handle->getParam("validator_file", validator_file);
    node_handle->getParam("policy_file", policy_file);
    node_handle->getParam("execution_files_path", execution_files_path);
    node_handle->getParam("planner_command", planner_command);
    node_handle->getParam("translator_command", translator_command);
    node_handle->getParam("validator_command", validator_command);
    node_handle->getParam("preprograph_command", preprograph_command);
    node_handle->param("prefVizType", prefVizType, std::string(".dot"));  // Either .dot or .pre
  }

  bool PROBPRPPlannerInterface::isFileAcessible(const char *fileName)
  {
    std::ifstream infile(fileName);
    return infile.good();
  }

  std::string PROBPRPPlannerInterface::getGraphFile(std::string viz_type)
  {
    std::stringstream graphFile;
    graphFile << execution_files_path << "/graph"<< viz_type;
    if (!isFileAcessible(graphFile.str().c_str()))
    {
      graphFile.clear();
  ...
  void PROBPRPPlannerInterface::parsePlan()
  {
    stateIDname.clear();
    statePolicy.clear();
    ...
    int curr, next, nodeCount, stateCount = 0;
    bool planFound = false, planRead = false;
    ...
    while (!infile.eof())
    {
      ...
      if (line.length() > 0)
      {
        ...
        {
          int stateIndexFROM = -1, stateIndexTO = -1;
          int switchstate = 0, count_start = 0, count_length = 0;
          std::string stateIndexFROMstr, stateIndexTOstr;
          for (std::string::iterator it = line.begin(); it != line.end(); ++it)
          {
            ...
            line.erase(std::remove(line.begin(), line.end(), ' '), line.end());
            line.erase(std::remove(line.begin(), line.end(), '\t'), line.end());
            switchstate = 0, count_start = 0, count_length = 0;
            std::string actionName, outcomeName;
            for (std::string::iterator it = line.begin(); it != line.end(); ++it)
            {
              switch (switchstate)
              {
              ...
              stateOutcome[std::pair<int, std::string>(stateIndexFROM, outcomeName)] = stateIndexTO;
            ...
            {
              int stateIndex = 0, switchstate = 0, count_start = 0, count_length = 0;
  ...
  /**
	 ...
	 */
  std::string PROBPRPPlannerInterface::runCommand(std::string cmd)
  {
    std::string data;
    FILE *stream;
    char buffer[1000];
   ...
   */
  bool PROBPRPPlannerInterface::runPlanner()
  {
    struct timeval stop, start;
    gettimeofday(&start, NULL);
    ROS_INFO("Running Planner\n %s", planner_command.c_str());
    runCommand(planner_command);
    gettimeofday(&stop, NULL);
    printf("took %lu\n", stop.tv_usec - start.tv_usec);

    gettimeofday(&start, NULL);
    ROS_INFO("\nTranslating Plan\n %s", translator_command.c_str());
    runCommand(translator_command);
    gettimeofday(&stop, NULL);
    printf("took %lu\n", stop.tv_usec - start.tv_usec);

    gettimeofday(&start, NULL);
    ROS_INFO("Generating Execution Files\n %s", validator_command.c_str());
    runCommand(validator_command);
    gettimeofday(&stop, NULL);
    printf("took %lu\n", stop.tv_usec - start.tv_usec);
    ...
    if (!prefVizType.compare(".pre"))
    {
        ...
        runCommand(preprograph_command);
        gettimeofday(&stop, NULL);
        printf("took %lu\n", stop.tv_usec - start.tv_usec);
    }
...
int main(int argc, char **argv)
{
  ...
  ros::NodeHandle nh("~");
  KCL_rosplan::PROBPRPPlannerInterface pi(nh);
  // subscribe to problem instance
  std::string problemTopic = "problem_instance";
  nh.getParam("problem_topic", problemTopic);
  ros::Subscriber problem_sub =
    nh.subscribe(
      problemTopic,
      1,
      &KCL_rosplan::PlannerInterface::problemCallback,
      dynamic_cast<KCL_rosplan::PlannerInterface*>(&pi));
  // start the planning services
  ros::ServiceServer service1 =
    nh.advertiseService(
      "planning_server",
      &KCL_rosplan::PlannerInterface::runPlanningServerDefault,
      dynamic_cast<KCL_rosplan::PlannerInterface*>(&pi));
  ros::ServiceServer service2
    = nh.advertiseService(
      "planning_server_params",
      &KCL_rosplan::PlannerInterface::runPlanningServerParams,
      dynamic_cast<KCL_rosplan::PlannerInterface*>(&pi));
